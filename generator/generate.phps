...
foreach (glob('../viathinksoft-std-*.txt') as $f) {
	...
	if ($std['is_markdown'] = file_exists(substr($f,0,-strlen('.txt')).'.md')) {
		$cont = file_get_contents(substr($f,0,-strlen('.txt')).'.md');
	} else {
...
// Create HTML page for each standard (currently only NROFF/TXT supported)
